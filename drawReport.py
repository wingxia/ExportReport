...
def initData():
    parser = argparse.ArgumentParser(
        description="This script is used to extract data from API/original report and call wordReport to generate a new report.")
    parser.add_argument("-m", "--model", dest="model", type=str, required=True,
                        choices=['api_awvs', 'html_awvs', 'html_goby', 'html_xray'],
                        help="选择模式:[api_awvs\html_awvs\html_goby\html_xray]")
    ...
    parser.add_argument("-o", "--out", dest="out", type=str,
                        help="输出文件路径 【默认：./result/{项目名称}渗透测试报告_{'生成时间'}.docx】")
